...
from src.main import _best_name, _display_results, _format_signal, _format_time, run_scan
...
class TestBestName:
    ...
    def test_device_name_preferred(self) -> None:
        device = Device(mac_address="AA:BB:CC:DD:EE:FF", device_type="wifi_ap", device_name="MyRouter", ssid="Home")
        assert _best_name(device) == "MyRouter"

    def test_ssid_when_no_name(self) -> None:
    ...
    def test_vendor_when_no_name_or_ssid(self) -> None:
        device = Device(mac_address="AA:BB:CC:DD:EE:FF", device_type="wifi_ap", vendor="TP-Link")
    ...
    def test_hidden_ssid_uses_vendor(self) -> None:
        device = Device(mac_address="AA:BB:CC:DD:EE:FF", device_type="wifi_ap", ssid="<Hidden>", vendor="Netgear")
...
class TestRunScan:
    ...
    def test_full_scan_with_mocked_scanners(
        ...
        # Mock WiFi scan returns one network
        ...
        # Mock Bluetooth scan returns one device
        from src.bluetooth_scanner import BluetoothDevice

        mock_bt_scan.return_value = [
            BluetoothDevice(mac_address="11:22:33:44:55:66", device_name="MockPhone")
        ]

        # Mock ARP scan returns one device
        from src.network_discovery import NetworkDevice

        mock_arp_scan.return_value = [
            NetworkDevice(ip_address="192.168.1.100", mac_address="AA:00:CC:DD:EE:FF", hostname="my-laptop")
        ]
